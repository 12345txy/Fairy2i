...
try:.configuration_Fairy_plus_minus_i import 
    from ComplexNetConfig
except ImportError:
...
class HalfComplexLinear(nn.Module):
    ...
    def reset_parameters(self):
        init.kaiming_uniform_(self.U_real, a=math.sqrt(5))
        init.kaiming_uniform_(self.U_imag, a=math.sqrt(5))
    ...
    def forward(self, x_real: torch.Tensor, x_imag: torch.Tensor) -> torch.Tensor:
        qW_real, qW_imag = self.weight_quantizer(self.W_real, self.W_imag)
        qU_real, qU_imag = self.weight_quantizer(self.U_real, self.U_imag)
        qx_real, qx_imag = self.act_quantizer(x_real, x_imag)

        out_real = F.linear(qx_real, qW_real + qU_real) + F.linear(qx_imag, qW_imag - qU_imag)
...
class ComplexLinear(nn.Module):
    ...
    def forward(self, x_real: torch.Tensor, x_imag: torch.Tensor) -> torch.Tensor:
        ...
        out_real = F.linear(qx_real, qW_real + qU_real) + F.linear(qx_imag, qW_imag - qU_imag)
        out_imag = F.linear(qx_real, qW_imag + qU_imag) + F.linear(qx_imag, -qW_real + qU_real)
